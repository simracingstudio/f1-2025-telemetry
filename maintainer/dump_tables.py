...
"""This script dumps the F1 2021 telemetry tables in ReST or MarkDown format."""

import os
import sys
import argparse

# Make sure we import from the f1_2021_telemetry package inside the repository.
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(sys.argv[0]), ".."))
)

from f1_2021_telemetry.packets import PacketID, EventStringCode
from f1_2021_telemetry.types import (
    ButtonFlag,
    DriverIDs,
    InfringementTypes,
    NationalityIDs,
...
def main():
    """Dump F1 2021 tables in ReST or MarkDown format."""

    argparser = argparse.ArgumentParser

    parser = argparse.ArgumentParser(
        description="Dump F1 2021 tables in format suitable for documentation."
    )
    parser.add_argument(
        "-f",
        "--format",
