...
"""This script dumps the F1 2022 telemetry tables in ReST or MarkDown format."""

import os
import sys
import argparse

# Make sure we import from the f1_2022_telemetry package inside the repository.
sys.path.insert(
    0, os.path.abspath(os.path.join(os.path.dirname(sys.argv[0]), ".."))
)

from f1_2022_telemetry.packets import PacketID, EventStringCode
from f1_2022_telemetry.types import (
    ButtonFlag,
    DriverIDs,
    InfringementTypes,
    NationalityIDs,
...
def dump_table_rst(items, labels, num_rows=None, num_cols=None):
    ...
    for col in range(num_cols):
        ...
        for row in range(num_rows):
            ...
            if idx < len(items):
                widths = tuple(
                    max(w, len(x)) for (w, x) in zip(widths, items[idx])
                )
    ...
    separator_line_1 = (
        "+"
        + "+".join(
            "+".join((w + 2) * "-" for w in widths) for widths in column_widths
        )
        + "+"
    )
    separator_line_2 = (
...
def dump_tables(dump_table_func):
    ...
    PacketIDTable = [
        (PacketID.short_description[p], p.value, PacketID.long_description[p])
        for p in PacketID
    ]
    dump_table_func(PacketIDTable, ["Packet Name", "Value", "Description"])
    print()

    EventStringCodeTable = [
        (
            EventStringCode.short_description[e],
            e.value.decode(),
            EventStringCode.long_description[e],
        )
        for e in EventStringCode
    ]
    ...
    ButtonFlagTable = [
        (f"0x{k:04x}", v)
        for (k, v) in sorted(
            (bf.value, ButtonFlag.description[bf]) for bf in ButtonFlag
        )
    ]
    ...
    dump_table_func(
        sorted(InfringementTypes.items()), ["ID", "Infringement meaning"]
    )
    print()
...
def main():
    """Dump F1 2022 tables in ReST or MarkDown format."""

    argparser = argparse.ArgumentParser

    parser = argparse.ArgumentParser(
        description="Dump F1 2022 tables in format suitable for documentation."
    )
    parser.add_argument(
        "-f",
        "--format",
